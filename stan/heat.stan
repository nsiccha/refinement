 ...
 transformed parameters {
   vector[N_meas] y_true;
   if(likelihood){
     y_true = solve_pde(T_meas / Nt_fit, Nx_fit, K, T_meas, x_meas);
   }
 }
 ...
 model {
   ...
   K ~ normal(0, 1.0);
   if(likelihood){
     y_observed ~ normal(y_true, sigma);
   }
 }
generated quantities {
  vector[N_meas] y_predicted = to_vector(normal_rng(
      solve_pde(T_meas / Nt_sim, Nx_sim, K, T_meas, x_meas),
      sigma
    ));
}
